...
logger = setup_logger(
    __name__, f'{__file__.split(".")[0]}.log', level=logging.INFO, quiet=False
)
        ...
        if cache == None:
            if not os.path.exists(cache_path):
                ...
                try:
                    ...
                    logger.error(
                        f"Error loading cache from {cache_path}, set cache to empty dict"
                    )
                    cache = {}

        if (cache_sign and key in cache) and not (
            cache[key] is None or cache[key] == ""
        ):
            return cache[key]
        else:
...
def get_response(model, messages, max_tokens=None, nth_generation=0):
    ...
    if model.startswith("claude") and messages and messages[0]["role"] == "system":
    ...
    try:
        ...
        client = openai.OpenAI(
            api_key=config["api_key"], base_url=config["base_url"], timeout=180
        )
    ...
    # logger.error error info
        ...
        logger.error(
            f"Number of input tokens: {num_tokens_from_string(messages[0]['content'])}"
        )
...
def add_speaker_name(dialogue: str, speaker: str) -> str:
    ...
    if any(
        line.strip().startswith(f"{speaker}:")
...
def ensure_scenes(cand_scenes, **kwargs):
    if (
        isinstance(cand_scenes, list)
        and len(cand_scenes) > 0
        and {
            "scenario",
            "actor_role",
            "user_role",
