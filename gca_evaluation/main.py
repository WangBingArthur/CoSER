...
parser.add_argument(
    "--regenerate", action="store_true", help="Regenerate the simulation results"
)
parser.add_argument(
    "--reevaluate", action="store_true", help="Reevaluate the simulation results"
)
parser.add_argument(
    "--nth_exp",
    type=int,
...
parser.add_argument(
    "--num_workers", type=int, default=1, help="Number of parallel workers (default: 1)"
)
...
def gca_simulation(test_file, actor_model, env_model, nsp_model, retrieval, nth_exp=0):
    ...
    logger.info(
        f"Conducting GCA Simulation for {actor_setting} on {test_file}\n\nThe results will be saved to {simulation_path}"
    )
    ...
    for circumstance in test_dataset:
        ...
        for character in speaking_characters_w_env:
            ...
            if character in plot_characters:
                character_info = [
                    c for c in plot["key_characters"] if c.get("name", "") == character
                ][0]
                if "description" in character_info:
                    character_profile = (
                        character_info.get("description", "").strip("\n")
                        + "\n\n"
        ...
        for character in speaking_characters_w_env + [NSP]:
            ...
            if character == NSP:
                ...
                if retrieval and character in book_database["character_datasets"]:
                    ...
                    involved_plots = (
                        [_["i_p"] for _ in character_database["plots"]]
                    ...
                    character_database["detailed_plots"] = [
                        book_database["plots"][i] for i in involved_plots
                    ]
                else:
                    character_database = None
            ...
            character_agent = Agent(
                ...
                retrieval_target=(
                    retrieval
                    if (retrieval and character not in special_characters)
                    else None
                ),
            )
            ...
            for actor in [current_speaker, "NSP"]:
                ...
                if args.continue_from > i_round:
                    if actor == current_speaker:
                        ...
                        response = (
                            conversation["dialogues"][i_round + 1]["character"]
                ...
                if actor == "NSP":
                    ...
                    next_actor = (
                        response.split(":")[0].strip() if ":" in response else response
                    )
                    ...
                    if next_actor == "<END CHAT>" and i_round >= 5:
                        ...
                        next_actor in speaking_characters_w_env
                        and next_actor != current_speaker
                    ):
                        current_speaker = next_actor
                    else:
                        # Fallback to random selection if prediction is invalid
                        candidates = set(major_characters + [ENVIRONMENT]) - {
                            current_speaker
                        }
                        if not candidates:
                            candidates = set(speaking_characters_w_env) - {
                                current_speaker
                            }
                    ...
                    logger.info(
                        f"Next speaker: {current_speaker} (Raw response: {response})"
                    )
                    ...
                    logger.info(
                        f"{env_model if actor == ENVIRONMENT else actor_model}: {response}\n"
                    )
...
def gca_judging(test_file, actor_model, retrieval, judge_model, nth_exp=0):
    ...
    logger.info(
        f"Evaluating GCA Simulation for {actor_setting} on {test_file}\n\nThe results will be saved to {evaluation_path}"
    )
    ...
    for result in simulation_results:
        book_title, i_p, i_c, circumstance, simulation = (
            result["book_title"],
            result["i_p"],
            result["i_c"],
            result["circumstance"],
        ...
        simulation = [
            (
                ...
                else {**m, "content": remove_inner_thoughts(m["content"])}
            )
            for m in simulation
        ]
        ...
        character_profile_str = "\n\n".join(
            [
                f"### {character}\n\n{profile.strip('')}"
                for character, profile in result["involved_character_profiles"].items()
            ]
        )
        ...
        for dimension in dimensions:
            ...
            critic_prompt = (
                critic_prompts["self-play-deduct-template"]
                .replace("{book}", book_title)
                .replace("{plot_summary}", circumstance["plot"]["summary"])
                ...
                .replace(
                    "{dimension_brief}",
                    critic_prompts["dimension_details"][dimension]["dimension_brief"],
                )
                .replace(
                    "{dimension_criteria}",
                    critic_prompts["dimension_details"][dimension][
                        "dimension_criteria"
                    ],
                )
            )
                ...
                min(
                    ...
                    - (
                        sum(
                            [
                                f["severity"]
                                for f in res[dimension]["flaws"]
                                if isinstance(f["severity"], int)
                            ]
                        )
                        - 0.3 * actor_rounds
                    )
        ...
        cases[f"{book_title}-{i_p}-{i_c}"] = {
            ...
            "score": sum([eval_result[dimension]["score"] for dimension in dimensions])
            / len(dimensions),
    ...
    avg_scores = {
        dimension: sum(scores[dimension]) / max(1, len(scores[dimension]))
        for dimension in dimensions
    }
    ...
    with open(evaluation_path, "w") as f:
        json.dump(
            {"scores": avg_scores, "cases": cases}, f, ensure_ascii=False, indent=2
        )
...
if __name__ == "__main__":
    ...
    for nth_exp in nth_exps:
        ...
        exp_name = "eval"
        if args.continue_from > 0:
            exp_name += f"-continue_from={args.continue_from}"
        if nth_exp > 0:
        ...
        def evaluate(exp_args):
            ...
            scores, cases = gca_judging(
                args.test_file, actor_model, args.retrieval, args.judge_model, nth_exp
            )
        ...
        actor_models = [
            args.actor_model
        ]  # you can modify the list to expand to multiple models
        ...
        # Sequential execution path
            ...
            for exp_arg in exp_args:
                ...
                # Create identifier for this model run
                actor_setting = (
                    f'{actor_model}{"_rag=" + args.retrieval if args.retrieval else ""}'
                )
