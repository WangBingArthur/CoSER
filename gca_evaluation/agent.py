...
class Embedding(Embeddings):
    ...
    def _embed(self, text: str) -> List[float]:
        ...
        text = text.replace("\n", " ")
        try:
            embedding = self.client.embeddings.create(input=[text], model=self.model)
...
def build_rag_corpus(character, database, target):
    ...
    for plot in database["detailed_plots"]:
        ...
        conversation_info = {
            ...
            "conversation": copy.deepcopy(plot["conversation"]),
        }
    ...
    corpus_map = {
        ...
        "expr3_conv1": [
            (experience_corpus, 3, "experience"),
            (conversation_corpus, 1, "conversation"),
        ],
        "expr10_conv1": [
            (experience_corpus, 10, "experience"),
            (conversation_corpus, 1, "conversation"),
        ],
    }
...
class Agent:
    ...
    def __init__(
        ...
        scene: Dict = None,
        retrieval_target: str = "conversation",
    ):
        ...
        if self.model.startswith("llama") or self.model.startswith("step"):
            self.system_prompt = (
                self.system_prompt
                + f'Start your response with "{name}: ". Avoid speaking as other characters.\n\n'
            )
    ...
    def chat(self) -> str:
        ...
        try:
                        ...
                        if current_character != character:
                            ...
                            if current_utterance:
                                parsed_utterances.append(
                                    (current_character, current_utterance)
                                )
            ...
            if (
                self.model.startswith("llama") or self.model.startswith("step")
            ) and self.name != "NSP":
            ...
            if (
                not any(
                    self.model.lower().startswith(model_type)
                    for model_type in ["gpt", "claude"]
                )
                and self.name != "NSP"
            ):
                ...
                if res:
                    logger.info(
                        f'{self.model} Repetition found and fixed: Original: "{response}" Fixed: "{res}"'
                    )
                    response = res
